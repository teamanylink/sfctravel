...
export default function TransactionTable() {
  ...
  return (
    <div className="bg-white rounded-[2rem] border border-navy-900/5 shadow-sm overflow-hidden">
      <div className="p-6 md:p-8 border-b border-navy-900/5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white">
        <h3 className="font-serif text-xl text-navy-900">Transaction History</h3>
        <button 
          onClick={handleExport}
          className="flex items-center gap-2 text-[10px] font-medium tracking-[0.1em] text-navy-900/60 uppercase hover:text-navy-900 transition-colors bg-white px-4 py-2 rounded-lg border border-navy-900/10 hover:bg-navy-50"
        >
          <Download size={14} />
          Export CSV
        </button>
      ...
      <div className="hidden md:block overflow-x-auto">
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-navy-900/5 text-[10px] font-medium tracking-[0.15em] text-navy-900/40 uppercase bg-white">
              <th className="px-8 py-5 font-normal">Date</th>
              <th className="px-8 py-5 font-normal">Description</th>
              <th className="px-8 py-5 font-normal text-right">Amount</th>
              <th className="px-8 py-5 font-normal text-right">Rebate Earned</th>
          ...
          <tbody className="divide-y divide-navy-900/5">
            {mockTransactions.map((tx) => (
              <tr key={tx.id} className="hover:bg-[#fafafa] transition-colors group">
                <td className="px-8 py-6 text-sm font-light text-navy-900/60 whitespace-nowrap">
                  {new Date(tx.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
                </td>
                <td className="px-8 py-6 text-sm font-medium text-navy-900">
                  {tx.description}
                </td>
                <td className="px-8 py-6 text-sm font-light text-navy-900 text-right whitespace-nowrap">
                  ${tx.amount.toLocaleString()}
                </td>
                <td className="px-8 py-6 text-sm font-medium text-green-600 text-right whitespace-nowrap">
                ...
                <td className="px-8 py-6 text-right whitespace-nowrap">
                  <span className={`inline-flex items-center justify-center px-3 py-1 rounded-full text-xs font-medium ${
                    tx.status === 'Completed' ? 'bg-[#f0fdf4] text-[#166534]' :
                    tx.status === 'Pending' ? 'bg-[#fefce8] text-[#854d0e]' :
                    'bg-[#f8fafc] text-navy-700'
                  }`}>
                    {tx.status}
                  </span>
                </td>
